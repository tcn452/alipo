        ...
        <section className="mx-auto grid max-w-[1440px] lg:min-h-[720px] lg:grid-cols-[440px_minmax(0,1fr)]">
          ...
          <div className={`${activeTab === 'list' ? 'hidden lg:block' : 'block'} relative min-h-[610px] bg-[#dce2d6] lg:min-h-[720px]`}>
            ...
            {selectedStation && <div className="absolute bottom-5 left-4 right-4 z-[400] border border-black/10 bg-white p-5 shadow-[0_24px_70px_rgba(5,48,33,.22)] sm:left-6 sm:right-auto sm:w-[410px]">
              ...
              <div className="mt-4 grid grid-cols-3 border-y border-line py-3 text-xs"><div><span className="block text-muted">Petrol</span><strong className="font-mono">MWK {selectedStation.latest_price_petrol?.toLocaleString() || '—'}</strong></div><div><span className="block text-muted">Diesel</span><strong className="font-mono">MWK {selectedStation.latest_price_diesel?.toLocaleString() || '—'}</strong></div><div><span className="block text-muted">Updated</span><strong>{formatTimeAgo(selectedStation.last_reported_at || selectedStation.updated)}</strong></div></div>
              <button onClick={() => setIsReportModalOpen(true)} className="mt-4 inline-flex w-full items-center justify-between bg-forest px-4 py-3 text-sm font-black text-white transition hover:bg-[#0b5940]">Report an update <ArrowRight className="h-4 w-4" /></button>
            </div>}
          </div>
        </section>
