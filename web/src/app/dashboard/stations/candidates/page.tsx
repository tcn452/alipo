...
export default function StationCandidatesPage() {
  ...
  const loadCandidates = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'Unable to load station candidates.');
      setCandidates(result.candidates || []);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : 'Unable to load station candidates.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const review = async (group: CandidateGroup, action: 'accept' | 'reject' | 'create') => {
    ...
    if (phone.replace(/\D/g, '').length < 7) {
      setError('Enter a valid phone number to vote.');
      return;
    }
    const prompt = action === 'reject'
      ? `Reject ${candidate.source_name}?`
      : action === 'create'
        ? `Create ${candidate.source_name} as a new live station at the proposed coordinates?`
        : `Match ${candidate.source_name} to ${candidate.nearest_station_name}?`;
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'Unable to review this candidate.');
      if (result.result?.confirmed) {
        setCandidates((current) => current.filter((row) => `${row.source}:${row.source_record_id}` !== group.key));
        setMessage(action === 'reject' ? 'Candidate rejected.' : action === 'create' ? 'New station created.' : 'Candidate matched to the existing station.');
      } else {
      ...
      setError(reviewError instanceof Error ? reviewError.message : 'Unable to review this candidate.');
    } finally {
      setWorkingKey(null);
    }
  };
  ...
  return (
    ...
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 className="text-2xl font-black tracking-tight text-gray-950">Candidate filling stations</h1>
        <p className="mt-1 max-w-2xl text-sm text-gray-500">Compare operator records with map results. During family testing, one community vote will confirm a match, reject a bad result, or create a genuinely missing station.</p>
      </div>
      <div className="flex gap-2"><Link href="/stations/add" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-800 px-4 text-xs font-black text-white"><Plus className="h-4 w-4" /> Add a station</Link><button type="button" onClick={() => void loadCandidates()} disabled={loading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-4 text-xs font-bold text-gray-700 hover:bg-gray-50 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Refresh</button></div>
    </header>

    <section className="flex flex-wrap items-center gap-2 border-y border-gray-200 py-3">
      {(['all', 'high', 'review'] as const).map((value) => <button key={value} type="button" onClick={() => setFilter(value)} className={`min-h-10 rounded-full px-4 text-xs font-bold ${filter === value ? 'bg-emerald-900 text-white' : 'bg-white text-gray-600 ring-1 ring-gray-200'}`}>{value === 'all' ? `All (${new Set(candidates.map((c) => `${c.source}:${c.source_record_id}`)).size})` : value === 'high' ? 'High confidence' : 'Needs review'}</button>)}
      <button type="button" onClick={locateUser} className="ml-auto inline-flex min-h-10 items-center gap-2 rounded-full bg-white px-4 text-xs font-bold text-gray-700 ring-1 ring-gray-200"><LocateFixed className="h-4 w-4 text-emerald-700" /> {userLocation ? 'Nearest first' : 'Sort by my distance'}</button>
    </section>
    ...
    <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 sm:flex sm:items-center sm:justify-between sm:gap-5">
      <div><p className="text-sm font-black text-emerald-950">Your phone number</p><p className="mt-1 text-xs text-emerald-800">Used only to prevent duplicate votes. It is converted to a private fingerprint before storage.</p></div>
    ...
    {loading ? <div className="grid min-h-72 place-items-center rounded-2xl border border-gray-200 bg-white"><span className="inline-flex items-center gap-2 text-sm font-bold text-gray-500"><RefreshCw className="h-4 w-4 animate-spin text-emerald-600" /> Loading candidate stations…</span></div> : groups.length === 0 ? <div className="grid min-h-72 place-items-center rounded-2xl border border-gray-200 bg-white text-center"><div><Check className="mx-auto h-8 w-8 text-emerald-600" /><p className="mt-3 font-black text-gray-900">No candidates in this view</p><p className="mt-1 text-sm text-gray-500">The review queue is clear for this filter.</p></div></div> : <div className="grid gap-4 lg:grid-cols-2">
      {groups.map((group) => {
        ...
        return <article key={group.key} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div className="flex items-start justify-between gap-4 border-b border-gray-100 p-5">
            <div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.14em] text-emerald-700">{c.operator_name} · {c.source_city || 'Malawi'}</p><h2 className="mt-1 truncate text-lg font-black text-gray-950">{c.source_name}</h2><p className="mt-1 text-xs leading-5 text-gray-500">{c.source_address || 'No official street address supplied'}</p>{userLocation ? <p className="mt-2 text-xs font-black text-emerald-700">{distanceKm(userLocation, c).toFixed(1)} km from you</p> : null}</div>
          ...
          {group.alternatives.length ? <div className="mx-5 mb-4 rounded-xl border border-gray-200 px-4 py-3"><p className="text-[10px] font-black uppercase tracking-wider text-gray-400">Other provider evidence</p>{group.alternatives.map((alt) => <p key={alt.id} className="mt-1.5 text-xs text-gray-600"><span className="font-bold capitalize">{alt.provider}</span>: {alt.result_name || 'Unnamed result'} · {alt.confidence_score}%</p>)}</div> : null}

          <div className="grid grid-cols-1 gap-2 border-t border-gray-100 bg-gray-50 p-4 sm:grid-cols-3">
            <button type="button" disabled={workingKey !== null} onClick={() => void review(group, 'accept')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-3 text-xs font-black text-white hover:bg-emerald-800 disabled:opacity-50"><Check className="h-4 w-4" /> Match</button>
            <button type="button" disabled={workingKey !== null || tooCloseToCreate} onClick={() => void review(group, 'create')} title={tooCloseToCreate ? 'Use Match when an existing pin is within 75 metres.' : undefined} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-gray-900 px-3 text-xs font-black text-white hover:bg-black disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-4 w-4" /> Create</button>
            <button type="button" disabled={workingKey !== null} onClick={() => void review(group, 'reject')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-red-200 bg-white px-3 text-xs font-black text-red-700 hover:bg-red-50 disabled:opacity-50"><X className="h-4 w-4" /> Reject</button>
          </div>
        </article>;
      })}
    </div>}
